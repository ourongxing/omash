...
| 内核管理 | 基础已实现 | 固定使用 Arch 的 `/usr/bin/mihomo`；用户级 supervisor、staging 校验、API readiness、热重载、崩溃退避拉起、日志；不支持外部 API 模式 |
| 连接 | 已实现 | 浏览、单条关闭、全部关闭 |
| 规则 | 已实现 | 规则列表 |
...
| TUN | 部分实现 | 可生成配置；尚未移植 Clash Verge Rev 特权服务，普通用户运行的系统核心不保证可启用 |
| 本地备份 | 已实现 | ZIP 创建、列出、确认恢复 |
| WebDAV | 待移植 | — |
| DNS/端口高级编辑 | 待移植 | — |
| Provider 管理 | Mihomo 托管 | omash 不主动维护；provider 的加载与更新周期完全由订阅配置和 Mihomo 负责 |
| 系统自启动 | Omarchy 已实现 | 包内 systemd 用户 unit；设置页控制图形登录自启动，旧版动态 unit 自动迁移 |
| 特权服务 | 待移植 | — |
| Mihomo 与 GeoData 维护 | 已实现 | Mihomo 版本交由 pacman/yay 维护；仅复用 Arch `clash-geoip` 的 `Country.mmdb`，不自行下载全局 GeoData |
